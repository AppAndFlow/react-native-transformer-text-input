...
} from 'react-native-transformer-input';
...
export default function App() {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior="padding">
      <Text style={styles.title}>React Native Transformer Input</Text>
      <View>
        ...
        <TransformerTextInput
          ...
          transformer={({ value }) => {
            ...
            const result =
              (cleanedInput ? '@' : '') +
              cleanedInput.toLowerCase().slice(0, 10);
            return { value: result };
          }}
          autoCapitalize="none"
          autoCorrect={false}
        ...
        <TransformerTextInput
          ...
          transformer={({
            ...
            // Format: +1 (XXX) XXX-XXXX (always with +1 prefix)
            ...
            if (deletedFormattingChar) {
              ...
              // Reformat with one fewer digit
              const area = trimmedDigits.slice(0, 3);
              const exchange = trimmedDigits.slice(3, 6);
              const subscriber = trimmedDigits.slice(6, 10);

              let trimmedFormatted = '+1 ';
              if (area.length > 0) {
                trimmedFormatted += '(' + area;
                if (area.length === 3) {
                  trimmedFormatted += ') ';
                }
              }
              if (exchange.length > 0) {
                trimmedFormatted += exchange;
                if (exchange.length === 3 && subscriber.length > 0) {
                  trimmedFormatted += '-';
                }
              }
              if (subscriber.length > 0) {
                trimmedFormatted += subscriber;
              }
            ...
            const clampedDigits = Math.min(
              digitsBeforeCursor,
              nationalDigits.length,
            );
            const newStart = positionAfterNationalDigit(
              formatted,
              clampedDigits,
            );
            ...
            const newEnd = positionAfterNationalDigit(
              formatted,
              clampedEndDigits,
            );
